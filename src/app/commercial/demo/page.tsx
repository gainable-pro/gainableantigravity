        ...
        <div className="max-w-4xl mx-auto space-y-6">
            ...
            <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 flex gap-3 text-amber-800">
                ...
                <div>
                    ...
                    <p className="text-sm mt-1">Vous ne pouvez vous connecter qu'aux comptes "Air G Énergie" et "SMB13". Il est strictement interdit de modifier des informations ou de supprimer des éléments dans ces espaces. Utilisez ces comptes uniquement pour de la consultation visuelle avec vos prospects.</p>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-8">
                ...
                <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6 flex flex-col justify-between">
                    <div>
                        <div className="flex items-center gap-3 mb-4">
                            <div className="h-12 w-12 bg-indigo-100 text-indigo-600 rounded-lg flex items-center justify-center font-bold text-xl">
                                S
                            </div>
                            <div>
                                <h2 className="text-xl font-bold text-slate-900">SMB13</h2>
                                <span className="inline-flex items-center gap-1 px-2 py-1 bg-emerald-100 text-emerald-700 rounded text-xs font-medium mt-1">
                                    <CheckCircle className="h-3 w-3" /> Compte validé actif
                                </span>
                            </div>
                        </div>
                        <p className="text-slate-500 text-sm mb-6">Utilisez ce compte pour démontrer la simplicité d'un profil axé sur la climatisation et les avis clients locaux.</p>
                    </div>

                    <button 
                        onClick={() => handleDemoLogin("SMB13")}
                        disabled={loading !== ""}
                        className="w-full flex justify-center items-center gap-2 py-2.5 px-4 rounded-lg font-medium text-white bg-slate-900 hover:bg-black transition-colors disabled:opacity-50"
                    >
                        {loading === "SMB13" ? <Loader2 className="h-5 w-5 animate-spin" /> : <PlayCircle className="h-5 w-5" />}
                        Se connecter (Démo)
                    </button>
                </div>
            </div>
