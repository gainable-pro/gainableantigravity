...
export function getDepartmentFromZip(zip: string): string {
    if (!zip || zip.length < 2) return "";

    // DOM-TOM logic (3 chars) vs Metropole logic (2 chars)
    const prefix3 = zip.substring(0, 3);
    if (DEPARTMENTS[prefix3]) return DEPARTMENTS[prefix3];

    const prefix2 = zip.substring(0, 2);
    return DEPARTMENTS[prefix2] || "";
}
